<script>
    ...
    async function handleBuy() {
        console.log(`Attempting to buy ${quantity} shares`);
        if (portfolioId) {
            try {
                const response = await fetch('/api/StockHoldingApi/BuyShares', {
                    method: 'POST',
                    headers: {
...
<main>
        ...
        <div class="purchase-section">
                ...
                <form on:submit|preventDefault={handleBuy}>
                    ...
                    <div class="input-group submit-group">
                        <button type="submit" class="btn btn-primary">Purchase {quantity} Share{ quantity !==1 ?'s':''}</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    <br/>
    <br/>
    <PortalPageMainSpecifiedFirstTicker firstTicker={ticker} {key}/>
